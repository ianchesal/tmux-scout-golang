...
func writeAtomic(path string, data []byte) error {
	...
	if err := os.WriteFile(tmp, data, 0644); err != nil { // #nosec G703 -- tmp path is constructed internally via fmt.Sprintf from a caller-controlled path, not from user input
		os.Remove(tmp)
		return err
	}
